...
int main(int argc, char *argv[] ) {
  ...
  int i,j;
  int mb= ceil(1.0 * N/nprocs);
  int cnt= N/nprocs;
  float matrix[mb*nprocs][N],matrix2[cnt][N];
  float vector[N];
  float result[mb*nprocs],result2[cnt];
  ...
  MPI_Scatter(matrix,mb*N,MPI_FLOAT,matrix2,mb*N,MPI_FLOAT,0,MPI_COMM_WORLD);
  ...
  MPI_Gather(result2,mb,MPI_FLOAT,result,mb,MPI_FLOAT,0,MPI_COMM_WORLD);
